...
//   }, [refresh, router, success])
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} css={loginPageStyles(error)}>
      <TextField label="メールアドレス" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
      <TextField label="パスワード" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
      <button className="LoginPage-Button" onClick={() => login(email, password)}>
        ログインする
        {loading && <CircularProgress size={20} className="LoginPage-Progress" />}
      </button>
    </form>
  )
}
